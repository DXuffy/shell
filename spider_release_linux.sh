...
mountVersionDir="/opt/mountDisk/${shareDir}"
...
mountVersionDisk() {
  ...
  m() {
    echo -e "\033[34m 正在挂载版本磁盘 \033[0m"
    mount -t cifs -o username=sunweb,password=LKhjk9\&*1541144 //172.16.0.116/test $mountVersionDir
    echo -e "\033[31m 版本磁盘已挂载 \033[0m"
  }
...
createVersionFile() {
  ...
  echo -e "\033[34m $releasePath\033[0m\033[31m => $newFileName 已生成 \033[0m"
}
...
getCompileProjects() {
  ...
  echo -e "\033[31m 您确定要自动上传吗？这将删除掉已经编译好的项目源码！(回车退出) \033[0m"
...
removeRelease() {
  ...
  then
    ...
    echo -e "\033[31m 初始化已完成\n 准备生成版本文件 \033[0m"
  fi 
}

# 提交文件至SVN
commitFile() {
  echo -e "\033[31m 准备提交代码至SVN \033[0m"
  ...
  echo -e "\033[31m 已提交编译代码至SVN, 先去下面这个地址拉包吧! \033[0m"
  echo -e "\033[31m http://172.16.0.116:8090/ViewFarmReport.aspx \033[0m"
  echo -e "\033[31m 接着, 去发邮件吧! 骚年 \033[0m"
}
...
getCurrentVersion() {
  ...
  v=${v##*/}
  v=${v%*.txt}
  echo -e "\033[31m 版本号 =>\033[0m\033[34m $v \033[0m"
}
